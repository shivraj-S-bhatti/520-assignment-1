...
def test_innovation_strategy(problem_name: str, strategy_name: str) -> Dict[str, Any]:
    ...
    problem_spec = pathlib.Path(f"problems/{problem_name}.md").read_text()
    ...
    # Test the code
    test_path = f"tests/test_{problem_name}.py"
    spec = importlib.util.spec_from_file_location(test_path.replace('/', '_').replace('.py', ''), test_path)
